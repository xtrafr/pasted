<script lang="ts">
	...
	let { open = $bindable(false), collection, onMessage }: Props = $props();
	...
	async function createShare() {
		...
		try {
			...
			copied = false;
			await loadShares();
			await onMessage?.('A collection share URL was created.');
		} catch (caught) {
	...
	async function revokeShare(shareId: string) {
		...
		try {
			...
			await onMessage?.('Collection share access revoked.');
		} catch (caught) {
...
<Dialog
	...
	description="Create a revocable read-only URL for this collection and its items."
	dismissible={!creating && !revokingId}
>
	<div class="share-panel">
		{#if createdUrl}
			<section class="created-share" aria-labelledby={`created-collection-share-${collection.id}`}>
				<div>
					<p class="eyebrow">Ready to copy</p>
					<h3 id={`created-collection-share-${collection.id}`}>This URL is shown once</h3>
				</div>
				<div class="copy-row">
					<input
						aria-label="New collection share URL"
						value={createdUrl}
						readonly
		...
		<section class="create-share" aria-labelledby={`create-collection-share-${collection.id}`}>
			<div>
				<p class="eyebrow">New URL</p>
				<h3 id={`create-collection-share-${collection.id}`}>Choose an optional expiry</h3>
			</div>
			<Input
		...
		<section class="active-list" aria-labelledby={`active-collection-shares-${collection.id}`}>
			<div class="section-heading">
				<div>
					<p class="eyebrow">Access</p>
					<h3 id={`active-collection-shares-${collection.id}`}>Active shares</h3>
				</div>
			...
			{:else}
				<p class="empty-copy">No active share URLs for this collection.</p>
			{/if}
		</section>
